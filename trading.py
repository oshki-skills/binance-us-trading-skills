...
PRICED_TYPES = {"LIMIT", "LIMIT_MAKER", "STOP_LOSS_LIMIT", "TAKE_PROFIT_LIMIT"}
STOPPED_TYPES = {"STOP_LOSS_LIMIT", "TAKE_PROFIT_LIMIT"}


def _validate(symbol, side, order_type, filters, last_price, quote_usd, quantity, price, stop_price=None):
    ...
    if order_type in PRICED_TYPES and not price:
        raise SafetyError(f"{order_type} needs a price.")
    if order_type in STOPPED_TYPES and not stop_price:
        raise SafetyError(f"{order_type} needs a stop_price (the trigger price).")

    ref = Decimal(str(price)) if (order_type in PRICED_TYPES and price) else last_price
    ...
    norm_price = norm_stop = None
    tick = Decimal(str(filters.get("tickSize", "0")))
    if order_type in PRICED_TYPES:
        p = Decimal(str(price))
        norm_price = (p / tick).to_integral_value(ROUND_DOWN) * tick if tick > 0 else p
    if order_type in STOPPED_TYPES and stop_price:
        sp = Decimal(str(stop_price))
        norm_stop = (sp / tick).to_integral_value(ROUND_DOWN) * tick if tick > 0 else sp
    return {"symbol": symbol, "side": side, "type": order_type, "quantity": qty,
            "price": norm_price, "stop_price": norm_stop, "ref_price": ref,
...
def _params(symbol, n, quote_usd):
    ...
    if n["type"] in PRICED_TYPES:
        p["price"] = str(n["price"])
        if n["type"] != "LIMIT_MAKER":
            p["timeInForce"] = "GTC"
    if n["type"] in STOPPED_TYPES and n.get("stop_price"):
        p["stopPrice"] = str(n["stop_price"])
    return p
...
def _preview_dict(n):
    return {"symbol": n["symbol"], "side": n["side"], "type": n["type"],
            ...
            "stop_price": (str(n["stop_price"]) if n.get("stop_price") else None),
            "ref_price": str(n["ref_price"]), "est_notional_usd": float(n["est_notional_usd"])}


# --- Single order ------------------------------------------------------------------------------
def preview_order(symbol, side, order_type, quote_usd=None, quantity=None, pct=None, price=None, stop_price=None):
    symbol = _sym(symbol)
    quantity = _resolve_sell_qty(symbol, side, quantity, pct)
    n = _validate(symbol, side, order_type, get_filters(symbol), get_price(symbol), quote_usd, quantity, price, stop_price)
    _signed("POST", "/api/v3/order/test", _params(symbol, n, quote_usd))
    return {"ok": True, "dry_run": True, "preview": _preview_dict(n)}


def place_order(symbol, side, order_type, quote_usd=None, quantity=None, pct=None, price=None, stop_price=None):
...
def _prep_leg(leg):
    ...
    n = _validate(symbol, leg["side"], leg.get("type", "LIMIT"), get_filters(symbol),
                  get_price(symbol), leg.get("quote_usd"), qty, leg.get("price"), leg.get("stop_price"))
...
def main():
    ...
    p.add_argument("--price", type=float); p.add_argument("--stop-price", dest="stop_price", type=float)
    ...
    try:
        if a.mode == "preview_order":
            out = preview_order(a.symbol, a.side, a.order_type, a.quote_usd, a.quantity, a.pct, a.price, a.stop_price)
        elif a.mode == "place_order":
            out = place_order(a.symbol, a.side, a.order_type, a.quote_usd, a.quantity, a.pct, a.price, a.stop_price)
